		...
		<div class="box">
			...
			<form name="patient" action="#" method="post" >
				...
				<div class="field ">
					...
					<div class="control">
						<input class="input is-large" readonly name="chartid" id="chartid"  value="<?php echo $this->General_model->ReturnRowValue("patients", "id", "id", "$chartid"); ?>" type="text" required>
					</div>
				</div>
				...
				<div class="field is-horizontal">
				  <div class="field-body">
					<div class="field">
					  ...
					  <p class="control">
						<input class="input is-large" maxlength="10" type="text" name="hrno" id="hrno"  value="<?php echo $this->General_model->ReturnRowValue("patients", "ohip", "id", "$chartid"); ?>">
				...
				<div class="field is-horizontal">
				  <div class="field-body">
					<div class="field" style="width: 310px;">
					  ...
					  <p class="control">
						<label class="radio">
						  <input type="radio" value="M" name="gender" id="gender" <?php echo $gender == "M" ? 'checked' : ''; ?> >
						  Male
						</label>
						<label class="radio ">
						  <input type="radio" value="F" name="gender" id="gender" <?php echo $gender == "F" ? 'checked' : ''; ?>>
						  Female
						</label>
					  </p>
					</div>
					<div class="field">
					  ...
					  <p class="control">
						...
						?>
						<input class="input is-large" type="text" name="dob" autocomplete="off" id="service_date" value="<?php echo $_dob; ?>">
				  ...
				  <div class="field-body">
					...
					<div class="field">
					  ...
					  <p class="control">
							<div class="select">
							<?php //echo $province; 
							//echo $plan = $this->General_model->ReturnRowValue("patients", "plan", "id", "$chartid");
							?>
							<select class="input is-large" name="plan" id="cmbplan" >
								<option>Select</option>
								<?php
							
									$plan = $this->General_model->ReturnRowValue("patients", "plan", "id", "$chartid");
									$List22 = $this->iacsmodel->ReturnArrayValue("tblplan", "provCode", "$province", "planCode", 'asc'); 	
									foreach($List22 as $ListRows2){
									?>
	...
	if(province =='ON')
	{
		$.ajax({
			...
			success: function(response) {
				...
				{
					$.ajax({
						url: '<?php echo base_url(); ?>admin/patient/update_ajax', // point to server-side controller method
						type: 'post',
						dataType: 'text',
						async: false,
						cache: false,
		...
		$.ajax({
			url: '<?php echo base_url(); ?>admin/patient/update_other_ajax', // point to server-side controller method
			type: 'post',
			dataType: 'text',
			async: false,
			cache: false,
